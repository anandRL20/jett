...
import { MessageCircle, ArrowUpRight, Gamepad2, Box } from "lucide-react";
import mbLogo from "@/assets/mbproductions-logo.jpg";
import jxLogo from "@/assets/jx3-logo.png";
...
const partners = [
  {
    logo: mbLogo,
    name: "MBPRODUCTIONS",
    tag: "Cinematic Productions",
    description:
      "Specializing in high-quality FiveM cinematics — bringing stories to life through stunning visual storytelling, action sequences, and cinematic video production.",
    icon: null,
    discord: null,
  },
  {
    ...
    tag: "3D Development",
    description:
      "Expert 3D development and asset creation for FiveM — custom models, props, and immersive environments crafted with precision and creativity.",
    icon: Box,
    discord: null,
  },
  {
  ...
  return (
    <section id="partners" className="py-32 px-6 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative">
        ...
        >
          Trusted partners we collaborate with across FiveM development, cinematics, and creative production.
        </motion.p>
        ...
        <div className="flex flex-col gap-0">
          {partners.map((c, i) => {
            ...
            return (
              ...
              >
                {/* Hover line */}
                <motion.div className="absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-primary/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                ...
                <div className="relative shrink-0">
                  {c.logo ? (
                    ...
                    <div className="w-16 h-16 rounded-full border border-border/40 bg-card/40 backdrop-blur-sm flex items-center justify-center">
                      {Icon && <Icon className="w-6 h-6 text-primary/70" />}
                    </div>
                  )}
                </div>
                ...
                {/* CTA — only shown if discord link exists */}
                {c.discord && (
                  <motion.a
                    href={c.discord}
                    target="_blank"
